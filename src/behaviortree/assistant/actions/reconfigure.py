...
def doReconfigure(args, autonomyRootLoc: str, buildTests: bool, buildDebug: bool):
    info(args, "Reconfiguring and building the behaviortree package...")
    
    #back out two directories for workspace path (autonomy root is <workspace>/src/behaviortree)
    ...
    cmd = "colcon build --cmake-clean-cache --packages-select behaviortree"
    if buildTests or buildDebug:
        cmd += " --cmake-args"
        
        if buildTests:
            info(args, "Building behaviortree with tests")
            cmd += " -DAUTONOMY_BUILD_TESTS=ON"
        
        if buildDebug:
            info(args, "Building behaviortree in debug mode")
