...
- Custom node *definitions* or *headers*, which are located in *include/behaviortree* and split up by node type (bt_actions, bt_conditions, and bt_decorators). This is the C++ code that defines the behaviors of the package's custom BT nodes.
- Custom node *tests*, which are located in *test/behaviortree* and split up by node type. This is the C++ code that tests the custom node definitions. All tests are written using the GTest framework. Header files for test utilities can be found in *include/behaviortree_test*.
...
- The BT assistant, located in *src/behaviortree/assistant* is a useful tool for performing tedious tasks related to the package.
...
```ros2 launch behaviortree autonomy.launch.py robot:=<robot name>```
...
```ros2 run behaviortree btassistant.py [-h] [-s] [-f]```

The tool can also be directly invoked by running the Python script located at:

*src/behaviortree/assistant/btassistant.py*
...
```ros2 run behaviortree btassistant.py create <action type> <node name> [--no-rebuild]```
...
```ros2 run behaviortree btassistant.py reconfigure_autonomy [--with-tests] [--debug]```
...
This subcommand is roughly equivalent to running ```colcon build --packages-select behaviortree``` with the necessary CMake flags set.
